...
export default function TvError({
  ...
  return (
    <main className="flex min-h-[60vh] flex-col items-center justify-center gap-6 px-6 text-center">
      ...
      <div>
        <p className="text-lg font-semibold text-white">Couldn&apos;t load this show</p>
        <p className="mt-1.5 max-w-sm text-sm text-white/50">
          {error.message?.includes("fetch") ? "A network error occurred while fetching show data." : error.message ?? "An unexpected error occurred."}
        </p>
      </div>
